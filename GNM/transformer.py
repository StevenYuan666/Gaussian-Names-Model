...
# Code from default implemention overwrites zero_attn which is needed for masked modeling
# This avoids reliance on <dummy> tokens
...
class _Transformer(nn.Module):
    def __init__(
        ...
        **layer_kwargs,
        ...
        super().__init__()
        self.d_model = layer_kwargs["d_model"]

        layer_kwargs["batch_first"] = True
        layer_kwargs["dim_feedforward"] = 2 * layer_kwargs["d_model"]
        if kind == "encoder":
            self.model = nn.TransformerEncoder(
                TransformerEncoderLayer(
                    activation=nn.functional.silu, **layer_kwargs
                ),  # type: ignore
                num_layers=num_layers,
            )
        elif kind == "decoder":
            self.model = nn.TransformerDecoder(
                nn.TransformerDecoderLayer(**layer_kwargs),
                num_layers=num_layers,
            )

    def zero_q(self):
        # get attention layer queries and set to 0
        # TODO check that the first d_model rows are the queries
...
class TransformerDecoder_custom(_Transformer):
    ...
    def forward(
        ...
        # memory: (batch_size, memory_len, d_model)
        # tgt_mask: (tgt_len, tgt_len)
        # memory_mask: (memory_len, memory_len)
        # tgt_key_padding_mask: (batch_size, tgt_len)
        # memory_key_padding_mask: (batch_size, memory_len)
        output = self.model(
            tgt,
            memory,
            tgt_mask,
